function AsMsNmpcCstr
%ASMSNMPCCSTR Summary of this function goes here
% 
% Implementation of Advanced-step Multistage Nonlinear Model Predictive
% Control (as-msNMPC) from Zhou (Joyce) Yu and Lorenz T. Biegler
%
% [OUTPUTARGS] = ASMSNMPCCSTR(INPUTARGS) Explain usage here
% 
% Examples: 
% 
...
% $Author: suwartad $	$Date: 2018/05/02 22:00:36 $	$Revision: 0.1 $
...
%[~, xmAs, umAs, obj, runtime] = spfNmpcCstr(@optProblem, @system, mpciterations, N, T, tmeasure, xmeasure, u0, scr);

% Debug version -- change when measurement available
[~, xmAs, umAs, obj, runtime] = spfNmpcCstrDebug(@optProblem, @system, mpciterations, N, T, tmeasure, xmeasure, u0, scr);

%save spfNmpcCstr.mat xmAs umAs;
%save spfNmpcCstrWoRT.mat xmAs umAs;
...
function [J,g,w0,w,lbg,ubg,lbw,ubw,params] = optProblem(x, u, N, x0_measure)   %add prediction horizon 
    ...
    % dimensions
    global nx nu nk d tf ns k scenario;
    nx = 2;   
    nu = 1;    
    nk = 1;
    %tf = 1;   % in [minutes]
...
function [J,g,w0,w,lbg,ubg,lbw,ubw,Xk,params,count,ssoftc] = iterateOnPredictionHorizon(Xk, w, w0, lbw, ubw, lbg, ubg, g, J, params, iter, count, ssoftc, xs)
   ...
   for k=0:nk-1
        ...
        % objective function with control input regularization 
