...
while(mpciter <= mpciterations)
    ...
    if mpciter == 1
        check = 0;
        %GOR   = paramModel.GOR;
        GOR   = paramModel.GOR_var;
    end
    
    if mpciter == 16
        check = 0;
        GOR  = paramModel.GOR + 0.5*ones(2,1);
    end
    
    if mpciter == 45
        check = 0;
        GOR   = paramModel.GOR + ones(2,1);
    end 
    ...
    [u_nlp_opt, x_nlp_opt, xuSS] = plotStatesGLSS(primalNLP, lb, ub, N);
    
    xuSSt = xuSS(31:end);
    
    if mpciter == 1 || mpciter == 16 || mpciter == 45
        check = 1;
        uf = xuSSt;
    end
