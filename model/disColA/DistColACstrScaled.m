function [t,states,xdot,inputs] = DistColACstrScaled(U)
...
for i=1:NT-1
    for j=1:NC-1
        if (i == 1) && (j == 1)
            y(i,j) = (xc*x(i,j)*alpha) / ( 1 + ((alpha-1)*xc*x(i,j)) );
        else
...
for j=1:NC-1
    if j == 1
        dMxdt(1,j)  = Li(2)*x(2,j) - Vi(1)*y(1,j) - B*xc*x(1,j);
    else
...
for i=1:NT+1
    for j=1:NC-1
        if (i==1) && (j==1)
            dxdt(i,j) = (dMxdt(i,j)-xc*x(i,j)*dMdt(i))/M(i);
        else
