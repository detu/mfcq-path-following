function [prob] = distColACstr_mfcq(p)
...
%     params.weights = weights;
    ...
    %Hc   = Function(c.hessian('V','c'));
    Hobj = f.hessian('V','objective');
    J    = c.jacobian('V','constraint');
    Jp   = cx0.jacobian('X0','constraint');
   
    % compute Hessian of the constraints.
    % since Hessian in CasADi is for scalar, we need to iterate for all
    % constraints
    %Hc = cell(nDual,nPrimal,nPrimal);
    
    % TO DO: - compute Hc ONE TIME ONLY! (ALSO H, and Jacobian of the
    % constraint !)
    global flagDt;
    if (flagDt == 1)
        %tic;
