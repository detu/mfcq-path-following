function [x_init, y_init, runtime, delta_t, success, etaData, numActiveBound, activeBoundTol] = solveThreeSteps1(prob, x_init, y_init, step, lb, ub, N, x0, t, delta_t, p_0, p_t, oldEta, ub_init, gOld)
...
% else
    ...
    while keepgoingrank
        [y_init, keepgoingrank, prevRankDef, count] = updateDualBasedOnRank(y_init, Jeq, prevRankDef, count);
        if keepgoingrank == 0
            break;
        end
    end
    ...
    [deltaXc,deltaYplus,activeBoundInd,JAbc,activeBoundTol,correctRt]      = solveCorrectStep(H, Hc, Jeq, g, cin, y_init, x_init, ub_init, oldEta);
    
%     % for DEBUGGING compute Eta just after the Corrector Step
%     % --- start of debugging ---
%     x_init         = x_init + deltaXc;
%     y_init.lam_g   = y_init.lam_g + deltaYplus.lam_g;
%     y_init.lam_x   = y_init.lam_x + deltaYplus.lam_x;
%     
%     flagDt         = 0;
%     [~,g,~,~,cin,~,~,Jeq,~,~,~] = prob.obj(x_init,y_init,p_t, N);
% %     flagDt         = 1;
% %     [~,g,H,Lxp,cin,~,~,Jeq,dpe,~,Hc] = prob.obj(x_init,y_init,p_t, N);
%     [newEta, z] = computeEta(Jeq, g, y_init, cin, activeBoundTol);
%     fprintf('newEta = %e \n',newEta);
%     
%     % run Corrector Step for the second time
%     [deltaXc,deltaYplus,activeBoundInd,JAbc,activeBoundTol]      = solveCorrectStep(H, Hc, Jeq, g, cin, y_init, x_init, ub_init, oldEta);
%     x_init         = x_init + deltaXc;
%     y_init.lam_g   = y_init.lam_g + deltaYplus.lam_g;
%     y_init.lam_x   = y_init.lam_x + deltaYplus.lam_x;
%     
...
%     fprintf('newEta = %e \n',newEta);
%     
%     % run Corrector Step for the third time
%     [deltaXc,deltaYplus,activeBoundInd,JAbc,activeBoundTol]      = solveCorrectStep(H, Hc, Jeq, g, cin, y_init, x_init, ub_init, oldEta);
%     x_init         = x_init + deltaXc;
%     y_init.lam_g   = y_init.lam_g + deltaYplus.lam_g;
%     y_init.lam_x   = y_init.lam_x + deltaYplus.lam_x;
...
%     fprintf('newEta = %e \n',newEta);
    ...
    % Second: Predictor
    %[deltaXp,deltaYp, qp_exit] = solveQPPredict(Hm, Jeq, g, gOld, cin, Hc, step, dpe, lb, ub, deltaXc, activeBoundInd);
    [deltaXp,deltaYp, predictRt] = solveQPPredict(H, Jeq, g, gOld, cin, Hc, step, dpe, lb, ub, deltaXc, activeBoundInd);
...
%             y_init.lam_g = deltaYp.lam_g;
%             y_init.lam_x = deltaYp.lam_x;
            ...
            %if numY+numActiveBound > numX
                ...
                % Third: Jump Step
                [lpSol, exitLP, linobj, jumpRT]   = solveJumpLP(Jeq, Lxp, g, dpe, cin, yCurrent, step, z, JAbc, activeBoundInd);
...
function [dXc,dYplus,activeIndex,JAbc,activeBoundTol] = solveCorrectStep(H, Hc, Jeq, g, c, y, x, ub, oldEta)
...
% % Hessian here includes hessian of objective function (H) AND constraints (Hc)!
% Hsc = spzeros(size(H));
% %Hst = spzeros(size(H));
% for i=1:numY
%     Hsc = Hsc + y.lam_g(i)*Hc{:,:,i};
%     %Hst = Hst + Hc{:,:,i};
% end
% Hm = H + Hsc;
Hm = H;
...
% lhs     = [Hm                   Jeq'; ...
%           Jeq                 zeros(n,n)];
%rhs     = -[g + Jm'*([y.lam_g; y.lam_x(activeIndex)]); [c;zeros(numActiveBoundCons,1)]];
...
function [dXp, dYp, qp_exit] = solveQPPredict(Hm, Jeq, g, gOld, cin, Hc, step, dpe, lb, ub, deltaXc, activeBoundInd)
